...
<html>
  ...
  <body>
  	<div class="mtitle">
		<span class="menu_selected">职位管理</span>
	</div>
   ...
   <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tabForm">
  	  ...
  	  <tr>
	  	<td colspan="2">
		  <form name="form1" method="post" action="role/selectRole">
  			<table width="100%" border="0" cellspacing="0" cellpadding="0" name="base" id="base">
			  <tr> 	
				<td width="30%" nowrap>职位名称模糊查询：
		  	    ...
		  	    <td width="25%" align="center">
					<div class="control">
						<button type="button" class="button" onMouseOver="this.className='button_over';" 
		      		    ...
		      		    <button type="button" class="button" onMouseOver="this.className='button_over';" 
		      		    		onMouseOut="this.className='button';" onClick="forward('role/findAllRole')">
	...
	<h3><img src="images/khlb.png" border="0">&nbsp;职位列表</h3>
	<div class="control">
		<button type="button" class="button" onMouseOver="this.className='button_over';" 
		        onMouseOut="this.className='button';" onClick="forward('role/addRole')">
	...
	<div class="border">
	  <form name="form2" method="post" action="role/deleteRole">
		<table width="100%" border="0" cellspacing="0" cellpadding="0" id="PowerTable" class="PowerTable">
		   ...
		   <c:if test="${not empty roleInfoList}">
		    	...
		    	<td width="4%" class="listViewThS1"><input type="checkbox" name="checkall"  id="checkall" onClick="checkAllBox();"/></td>
		  	    <td width="15%" class="listViewThS1">职位名称</td>
		  	    <td width="15%" class="listViewThS1">职位描述</td>
		  	    <td width="15%" class="listViewThS1">所属部门</td>
		  	    <td width="7%" class="listViewThS1">职位等级</td>
			  </tr>
		   	  <c:forEach items="${roleInfoList}" var="roleInfo">
				<tr>
			    	<td><input type="checkbox" name="ids" value="${roleInfo.roleId}" onClick="changeCheckCount();"/></td>
			  	    <td><a href="role/findroleInfo?roleId=${roleInfo.roleId}">${roleInfo.roleName}</a></td>
					<td>${roleInfo.roleDescription}</td>
					<td>${roleInfo.dpmName}</td>
					<td>${roleInfo.roleLevel}</td>
				</tr>
			  </c:forEach>
			</c:if>
   ...
   <!-- 分页显示所有数据 -->
   <c:if test="${roleName==null&&dpmName==null}">
	    ...
	    <tr>  
	     <td colspan="9">
	        <c:if test="${page.totalPage!=0}">  
	            ...
	            <a href="role/findAllRole?pageNow=1"><u>首页</u></a>  
	        	 	        	 
	       		<c:if test="${page.isHasPre()!=false}">	        		
	            	<a href="role/findAllRole?pageNow=${page.pageNow-1}"><u>上一页</u></a>
	        	</c:if>
	        	  
	        	<c:if test="${page.isHasNext()!=false}">	        		
	        		<a href="role/findAllRole?pageNow=${page.pageNow+1}"><u>下一页</u></a>            
	        	</c:if>
	        	  		         
		        <a href="role/findAllRole?pageNow=${page.totalPage}"><u>尾页</u></a>  
	    ...
	    <tr>  
	     <td colspan="9">
	        <c:if test="${page.totalPage!=0}">  
	            ...
	            <a href="role/selectRole?roleName=${roleName}&dpmName=${dpmName}&pageNow=1" ><u>首页</u></a>  
	        	 	        	 
	       		<c:if test="${page.isHasPre()!=false}">	        		
	            	<a href="role/selectRole?roleName=${roleName}&dpmName=${dpmName}&pageNow=${page.pageNow-1}"><u>上一页</u></a>
	        	</c:if>
	        	  
	        	<c:if test="${page.isHasNext()!=false}">	        		
	        		<a href="role/selectRole?roleName=${roleName}&dpmName=${dpmName}&pageNow=${page.pageNow+1}"><u>下一页</u></a>            
	        	</c:if>
	        	  		         
		        <a href="role/selectRole?roleName=${roleName}&dpmName=${dpmName}&pageNow=${page.totalPage}"><u>尾页</u></a>  
